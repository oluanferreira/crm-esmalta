...
function segments(d){let r=work(d);if(!r)return[];let as=A.filter(a=>a.date===d&&!['cancelled','missed'].includes(a.status)).sort((a,b)=>a.time.localeCompare(b.time)),out=[],cur=r[0];as.forEach(a=>{let s=tmin(a.time),e=s+P[a.procedure].min;if(s>cur)out.push({k:'free',s:cur,e:s});out.push({k:'appt',a,s,e});cur=Math.max(cur,e)});if(cur<r[1])out.push({k:'free',s:cur,e:r[1]});return out}
function firstSlot(d,proc){for(let i=0;i<21;i++){let x=add(d,i),dur=P[proc].min;for(let s of segments(x))if(s.k==='free'&&s.e-s.s>=dur)return{date:x,time:mt(s.s)}}return null}
...
function weekData(start=wstart(today())){let end=add(start,5),apps=A.filter(a=>a.date>=start&&a.date<=end&&!['cancelled','missed'].includes(a.status)),rs=returns().filter(r=>r.notify>=start&&r.notify<=end&&!future(r)),react=returns().filter(r=>r.type==='react'&&r.due<=end&&!future(r)),daysFree=Array.from({length:6},(_,i)=>{let d=add(start,i),segs=segments(d).filter(s=>s.k==='free'&&s.e-s.s>=50);return{date:d,minutes:segs.reduce((n,s)=>n+s.e-s.s,0),gaps:segs}});return{start,end,apps,rs,react,daysFree,rev:apps.reduce((s,a)=>s+P[a.procedure].price,0)}}
...
function valid(form,ignore){let p=P[form.procedure],r=work(form.date);if(!r)return'Marina não atende aos domingos.';let s=tmin(form.time),e=s+p.min;if(s<r[0]||e>r[1])return'Esse procedimento ultrapassa o horário de atendimento.';let c=A.find(a=>String(a.id)!==String(ignore)&&a.date===form.date&&!['cancelled','missed'].includes(a.status)&&s<tmin(a.time)+P[a.procedure].min&&e>tmin(a.time));return c?`Conflita com ${c.client}, às ${c.time}.`:''}